...
public class Monos {
    ...
    public static <T> Function<Mono<T>, Mono<T>> precondition(Predicate<T> validator, String errorMsg) {
        return precondition(validator, $ -> new IllegalStateException(errorMsg));
    }
    
    public static <T> Function<Mono<T>, Mono<T>> precondition(Predicate<T> validator, Function<T, ? extends Throwable> exceptionFactory) {
        return in -> in.handle((t, sink) -> { if (validator.test(t)) sink.next(t); else sink.error(exceptionFactory.apply(t)); });
    }
}
